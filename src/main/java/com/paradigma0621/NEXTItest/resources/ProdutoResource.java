...
public class ProdutoResource {
	...
	@RequestMapping(value = "/{id}", method = RequestMethod.GET) // Para dizer que o endpoint agora
																//é: /produtos/{id}
	public ResponseEntity<Produto> find(@PathVariable Integer id) { // Para o Spring saber que esse 
		//id informado nessa linha é o "id" da linha de cima colocamos o @PathVariable
	...
	public ResponseEntity<Void> insert(@RequestBody Produto obj) { // para que o objeto Produto 
	//seja construido a partir dos dados json que enviar, temos que inserir a notação '@RequestBody' -
	//ela faz o json ser convertido para o objeto Java automaticamente
		obj = service.insert(obj);

		// Quando inserir com sucesso: é retornado o código HTTP 201, juntamente com a
		// URI do novo objeto inserido
		URI uri = ServletUriComponentsBuilder.fromCurrentRequest() // Pega URI do novo recurso 
		//inserido = http://localhost:8080/produtos/
	...
	public ResponseEntity<Void> update(@RequestBody Produto obj, @PathVariable Integer id) { 
		// Para que o objeto Produto seja construido a partir dos dados json que enviar, temos que
	...
	public ResponseEntity<Void> delete(@PathVariable Integer id) { // 'Void': pois quando
	// apagar vai retornar uma resposta com corpo vazio. O @PathVariable é para o Spring 
	// saber que esse id informado nessa linha é o "id" da linha de cima
		service.delete(id);
		return ResponseEntity.noContent().build(); // Retorno de resposta HTTP ok
	}

	@RequestMapping(method = RequestMethod.GET) // endpoint = "http://localhost:8080/produtos")
												//(Busque todos produtos)
	public ResponseEntity<List<Produto>> findAll() {
		List<Produto> list = service.findAll();
		return ResponseEntity.ok().body(list); // ok() para dizer que a operação ocorreu com 
		//sucesso, e a resposta terá como corpo a 'list'
	}
